 ...
 */
 ...
 namespace Tqdev\PhpCrudApi {
     ...
     class Api implements RequestHandlerInterface
     {
         ...
         public function __construct(Config $config)
         {
             ...
             $prefix = sprintf('fgapirest-%s-', substr(md5(__FILE__), 0, 8));
 ...
 namespace Tqdev\PhpCrudApi {
  ...
  $config = new Config([
    ...
    'basePath' => "/pacmec-api",
    //'basePath' => "/home/dev/web/admin.pacmec.managertechnology.com.co/public_html",
    //'basePath' => "cache_user_sessions",
    #'basePath' => "/home/dev/tmp",
    'debug' => true,
 		'middlewares' => 'cors,sslRedirect,ajaxOnly,dbAuth,pageLimits,joinLimits,authorization,sanitation,validation,customization,multiTenancy', //  xsrf
 		'dbAuth.mode' => 'optional',
 		'dbAuth.usersTable' => 'users',
 		'dbAuth.usernameColumn' => 'username',
 		'dbAuth.passwordColumn' => 'hash',
 		'dbAuth.registerUser' => 1,
 		'dbAuth.passwordLength' => 8,
 		//'dbAuth.sessionName' => 8,
 		'pageLimits.pages' => 100,
 		'pageLimits.records' => 1000,
 		'joinLimits.depth' => 5,
 		'joinLimits.tables' => 10,
      ...
      if(isset($_SESSION['user'][$operation])) {
        $explode = explode(',', $_SESSION['user'][$operation]);
          ...
          return false;
 			  ...
 			  $file_path = PACMEC_PATH . "api/authorization/tableHandler/{$tableName}.{$operation}.php";
   			if(!is_file($file_path) || !@file_exists($file_path)){
   				$file_path = PACMEC_PATH . "api/authorization/tableHandler/default.{$operation}.php";
   				if(!is_file($file_path) || !@file_exists($file_path)){
   					return false;
   				}
   			} else {
    ...
    'authorization.columnHandler' => function ($operation, $tableName, $columnName) {
      ...
      if(!isset($_SESSION['user']['is_admin']) && !empty($_SESSION['user']['is_admin']) && $_SESSION['user']['is_admin'] == 1){ return true; }
 			$file_path = PACMEC_PATH . "api/authorization/recordHandler/{$tableName}.{$operation}.php";
 			if(!is_file($file_path) || !@file_exists($file_path)){
 				$file_path = PACMEC_PATH . "api/authorization/recordHandler/default.{$operation}.php";
 				if(!is_file($file_path) || !@file_exists($file_path)){
 					return false;
 				}
 			} else {
