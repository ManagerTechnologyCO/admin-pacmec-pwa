 ...
 */
?>
<div id="me-affiliates">
  <div v-if="affiliates!==null">
  	<v-style type="text/css">
  	</v-style>
  	<div class="dashboard-title fl-wrap">
  		<h3>{{translateField('history_recharges')}}</h3>
  	</div>
  	<div class="profile-edit-container fl-wrap block_box" v-if="affiliates.length==0">
      ...
      <div class="dashboard-list-box  fl-wrap pacmec-responsive">
				<table class="pacmec-table pacmec-bordered">
					...
					<tbody>
						<tr v-for="(affiliate, affiliate_i) in affiliates">
							<td>{{translateField('membership_'+affiliate.membership)}}</td>
							<td>{{translateField('affiliates_status_'+affiliate.status)}}</td>
							<td>{{parseFloat(affiliate.billing_amount).toLocaleString()}} {{(affiliate.initial_payment>0)?' + '+affiliate.initial_payment:''}}</td>
							<td>{{affiliate.startdate}}</td>
							<td>{{affiliate.enddate}}</td>
						</tr>
					</tbody>
				</table>
		</div>
...
var me_profile_change_pass = new Vue({
    ...
    loadHistory(){
			let self = this;
			PACMEC.pagination('affiliates', {}, (a) => {
				if(a.response){
					self.affiliates = a.response;
				}
			});
		},
	},
